...
import { Plus, Trash2, Save, FileDown, Package, X } from "lucide-react";
import { toast } from "sonner";
import { exportPDF } from "@/lib/pdf-export";
import { type Client, type Product, type SavedDocument, type DocumentVersion, saveDocument, addVersionToDocument } from "@/lib/storage";
...
export function DocumentForm({ selectedClient, editingDocument, onClearEdit, onDocumentSaved }: DocumentFormProps) {
  ...
  // Auto-fill assignor when client changes
  useEffect(() => {
  ...
  // Load document version for editing
  useEffect(() => {
    if (editingDocument) {
  ...
  const handleSave = () => {
    ...
    if (editingDocument) {
      // Save as new version of existing document
  ...
  return (
    <div className="space-y-4 pb-28 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold tracking-tight text-foreground">
            {isEditing ? "Редакция на документ" : "Нов документ"}
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            {isEditing
              ? `Редакция на v${editingDocument.doc.versions[editingDocument.versionIndex].version} → ще създаде v${editingDocument.doc.versions.length + 1}`
              : selectedClient
        ...
        {isEditing && (
          <Button variant="ghost" size="icon" className="h-9 w-9" onClick={resetForm}>
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
      ...
      <Card className="card-elevated">
        ...
        <CardContent className="space-y-4 px-4 pb-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input className="h-11" placeholder="Избери/създай клиент" value={assignor} onChange={(e) => setAssignor(e.target.value)} />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea rows={3} value={protocolText} onChange={(e) => setProtocolText(e.target.value)} className="resize-none text-sm" />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4 px-4 pb-4">
          <div className="space-y-3 p-3 rounded-lg bg-muted/50 border border-dashed">
            <div className="space-y-1.5">
              ...
              <Input className="h-11" placeholder="Име на продукта" value={newProduct.name} onChange={(e) => setNewProduct({ ...newProduct, name: e.target.value })} />
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div className="space-y-1.5">
                <Label className="text-xs">К-во</Label>
                <Input className="h-11" type="number" min={1} value={newProduct.quantity} onChange={(e) => setNewProduct({ ...newProduct, quantity: Number(e.target.value) })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Мярка</Label>
                <Input className="h-11" value={newProduct.unit} onChange={(e) => setNewProduct({ ...newProduct, unit: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Цена</Label>
                <Input className="h-11" type="number" min={0} step={0.01} value={newProduct.price} onChange={(e) => setNewProduct({ ...newProduct, price: Number(e.target.value) })} />
              </div>
            </div>
          ...
          {products.length === 0 ? (
            ...
            <div className="space-y-2">
              {products.map((p) => (
                <div key={p.id} className="flex items-center gap-3 p-3 rounded-lg bg-card border animate-fade-in">
                  ...
                  <Button variant="ghost" size="icon" className="h-9 w-9 shrink-0" onClick={() => removeProduct(p.id)}>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-md border-t p-3 flex gap-2 safe-bottom">
        <Button variant="outline" className="flex-1 h-12 gap-1.5 text-sm" onClick={handleExport}>
          <FileDown className="h-4 w-4" />
          PDF
        </Button>
        <Button className="flex-1 h-12 gap-1.5 text-sm" onClick={handleSave}>
